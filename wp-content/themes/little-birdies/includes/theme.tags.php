...
if ( !function_exists('little_birdies_need_page_title') ) {
	function little_birdies_need_page_title() {
		return little_birdies_get_theme_option('show_page_title') && !is_front_page() && apply_filters('little_birdies_filter_need_page_title', true);
	}
}
...
if ( !function_exists('little_birdies_show_layout') ) {
	function little_birdies_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('little_birdies_get_logo_image') ) {
	function little_birdies_get_logo_image($type='') {
		$logo_image = '';
		if (little_birdies_get_retina_multiplier(2) > 1)
			$logo_image = little_birdies_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) 
			$logo_image = little_birdies_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
		return $logo_image;
	}
}

// Return header video (if set)
if ( !function_exists('little_birdies_get_header_video') ) {
	function little_birdies_get_header_video() {
		$video = '';
		if (apply_filters('little_birdies_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (little_birdies_check_theme_option('header_video')) {
				$video = little_birdies_get_theme_option('header_video');
...
if ( !function_exists('little_birdies_show_post_meta') ) {
	function little_birdies_show_post_meta($args=array()) {
		...
		?><div class="post_meta"><?php
            ...
            if ( !empty($args['author'])) {		// && !is_attachment() && !is_page() ) {
                ...
                <a class="post_meta_item post_author" rel="author" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>">
                    <?php echo esc_html__('by', 'little-birdies') . ' ' . esc_html(get_the_author()); ?>
                </a>
            <?php
            }
			// Post categories
			...
			if ( !empty($args['date'])) {		// && !is_attachment() && !is_page() ) {
				$dt = apply_filters('little_birdies_filter_get_post_date', little_birdies_get_date());
				if (!empty($dt)) {
					?>
			...
			// Post counters
			little_birdies_show_layout(little_birdies_get_post_counters($args['counters']));

			// Socials share
			if ( !empty($args['share']) ) {
				little_birdies_show_share_links(array(
						'type' => 'drop',
						'caption' => esc_html__('Share', 'little-birdies'),
			...
			// Edit page link
			if ( !empty($args['edit']) ) {
				edit_post_link( esc_html__( 'Edit', 'little-birdies' ), '<span class="post_meta_item post_edit icon-pencil">', '</span>' );
			}
		?></div><!-- .post_meta --><?php
...
if ( !function_exists('little_birdies_show_post_featured') ) {
	function little_birdies_show_post_featured($args=array()) {

		$args = array_merge(array(
			'hover' => little_birdies_get_theme_option('image_hover'),	// Hover effect
			...
			'seo' => little_birdies_is_on(little_birdies_get_theme_option('seo_snippets')),
			'singular' => is_singular()						// Current page is singular (true) or blog/shortcode (false)
			), $args);

		if ( post_password_required() ) return;

		$thumb_size = !empty($args['thumb_size']) ? $args['thumb_size'] : little_birdies_get_thumb_size(is_attachment() ? 'full' : (is_single() ? 'huge' : 'big'));
		...
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				...
				$image = little_birdies_get_post_image();
				if (!empty($image)) 
					$image = little_birdies_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = little_birdies_get_no_image();
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . little_birdies_add_inline_style('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>"<?php if ($args['seo']) echo ' itemscope itemprop="image" itemtype="http://schema.org/ImageObject"'; ?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$little_birdies_attr = little_birdies_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
			...
			if ($has_thumb || !empty($args['show_no_image']) || (!$args['thumb_only'] && in_array($post_format, array('gallery', 'image', 'audio', 'video')))) {
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						little_birdies_hovers_add_icons($args['hover']);
					}
					...
					if ( ($output = little_birdies_build_slider_layout(array('thumb_size' => $thumb_size, 'controls' => 'yes', 'pagination' => 'yes'))) != '' )
						little_birdies_show_layout($output);
					...
					if (!empty($image)) {
						...
						<div class="mask"></div>
						<?php 
						little_birdies_hovers_add_icons($args['hover'], array('image' => $image));
					}
				} else if (!empty($args['show_no_image']) && ($image=little_birdies_get_no_image())!='') {
					?>
				...
				if ($post_format == 'video' && !$args['thumb_only']) {
					$video = little_birdies_get_post_video('', false);
					if (empty($video))
						$video = little_birdies_get_post_iframe('', false);
					if (!empty($video)) {
						if ( $has_thumb ) {
							$video = little_birdies_make_video_autoplay($video);
							?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
						}
						?><div class="post_video video_frame"><?php 
							if ( !$has_thumb ) {
								little_birdies_show_layout($video);
							}
						?></div><?php
					}
				...
				if ($post_format == 'audio' && !$args['thumb_only']) {
					$audio = little_birdies_get_post_audio('', false);
					if (empty($audio))
						$audio = little_birdies_get_post_iframe('', false);
					if (!empty($audio)) {
						//Show metadata (for the future version)
						if (false && function_exists('wp_read_audio_metadata')) {
							$src = little_birdies_get_post_audio($audio);
						...
						?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
							...
							$media_author = little_birdies_get_theme_option('media_author', '', false, get_the_ID());
							$media_title = little_birdies_get_theme_option('media_title', '', false, get_the_ID());
							if ( !empty($media_author) && !little_birdies_is_inherit($media_author) ) {
								?><div class="post_audio_author"><?php little_birdies_show_layout($media_author); ?></div><?php
							}
							if ( !empty($media_title) && !little_birdies_is_inherit($media_title) ) {
								?><h5 class="post_audio_title"><?php little_birdies_show_layout($media_title); ?></h5><?php
							}
							// Display audio
							little_birdies_show_layout($audio); 
						?></div><?php
					}
				}
				
				// Put optional info block over the thumb
				little_birdies_show_layout($post_info);
				?></div><?php
			}
		}
	}
...
if ( !function_exists('little_birdies_get_no_image') ) {
	function little_birdies_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = little_birdies_get_theme_option( 'no_image' );
			// Remove false in next line if you want display theme-specific image in the posts without featured image
			if (false && empty($img)) $img = little_birdies_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('little_birdies_add_bg_in_post_nav') ) {
	function little_birdies_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = little_birdies_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), little_birdies_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), little_birdies_get_thumb_size('med') );
				$img = $img[0];
			} else
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'little_birdies-main', $css );
	}
}

// Show related posts
if ( !function_exists('little_birdies_show_related_posts') ) {
	function little_birdies_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
			...
			<section class="related_wrap">
				<h2 class="section title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'little-birdies');
				?></h2>
...
if ( !function_exists('little_birdies_show_portfolio_posts') ) {
	function little_birdies_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? little_birdies_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = little_birdies_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = little_birdies_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the little_birdies_ajax_get_posts action
if ( !function_exists( 'little_birdies_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_little_birdies_ajax_get_posts',			'little_birdies_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_little_birdies_ajax_get_posts',	'little_birdies_ajax_get_posts_callback');
	function little_birdies_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			little_birdies_storage_set('blog_mode', 'blog');
			little_birdies_storage_set('options_meta', get_post_meta($id, 'little_birdies_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => little_birdies_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'little-birdies');
		}
...
// Show pagination
if ( !function_exists('little_birdies_show_pagination') ) {
	function little_birdies_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = little_birdies_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'little-birdies' ),
				'next_text' => esc_html__( '>', 'little-birdies' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'little-birdies' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'little-birdies'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'little-birdies') : esc_html__('Newest posts', 'little-birdies') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'little-birdies') : esc_html__('Older posts', 'little-birdies'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
