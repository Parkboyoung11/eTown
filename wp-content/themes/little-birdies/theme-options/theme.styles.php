...
if (!function_exists('little_birdies_customizer_add_theme_colors')) {
	function little_birdies_customizer_add_theme_colors($colors) {
		if (substr($colors['text'], 0, 1) == '#') {
			$colors['bg_color_0']  = little_birdies_hex2rgba( $colors['bg_color'], 0 );
			$colors['bg_color_02']  = little_birdies_hex2rgba( $colors['bg_color'], 0.2 );
            $colors['bg_color_03']  = little_birdies_hex2rgba( $colors['bg_color'], 0.3 );
			$colors['bg_color_07']  = little_birdies_hex2rgba( $colors['bg_color'], 0.7 );
            $colors['bg_color_06']  = little_birdies_hex2rgba( $colors['bg_color'], 0.6 );
			$colors['bg_color_08']  = little_birdies_hex2rgba( $colors['bg_color'], 0.8 );
			$colors['bg_color_09']  = little_birdies_hex2rgba( $colors['bg_color'], 0.9 );
			$colors['alter_bg_color_07']  = little_birdies_hex2rgba( $colors['alter_bg_color'], 0.7 );
			$colors['alter_bg_color_04']  = little_birdies_hex2rgba( $colors['alter_bg_color'], 0.4 );
			$colors['alter_bg_color_02']  = little_birdies_hex2rgba( $colors['alter_bg_color'], 0.2 );
			$colors['alter_bd_color_02']  = little_birdies_hex2rgba( $colors['alter_bd_color'], 0.2 );
			$colors['text_dark_07']  = little_birdies_hex2rgba( $colors['text_dark'], 0.7 );
			$colors['text_link_02']  = little_birdies_hex2rgba( $colors['text_link'], 0.2 );
			$colors['text_link_07']  = little_birdies_hex2rgba( $colors['text_link'], 0.7 );
...
if (!function_exists('little_birdies_customizer_add_theme_fonts')) {
	function little_birdies_customizer_add_theme_fonts($fonts) {
		$rez = array();	
		foreach ($fonts as $tag => $font) {
			//$rez[$tag] = $font;
			if (substr($font['font-family'], 0, 2) != '{{') {
				$rez[$tag.'_font-family'] 		= !empty($font['font-family']) && !little_birdies_is_inherit($font['font-family'])
														? 'font-family:' . trim($font['font-family']) . ';' 
														: '';
				$rez[$tag.'_font-size'] 		= !empty($font['font-size']) && !little_birdies_is_inherit($font['font-size'])
														? 'font-size:' . little_birdies_prepare_css_value($font['font-size']) . ";"
														: '';
				$rez[$tag.'_line-height'] 		= !empty($font['line-height']) && !little_birdies_is_inherit($font['line-height'])
														? 'line-height:' . trim($font['line-height']) . ";"
														: '';
				$rez[$tag.'_font-weight'] 		= !empty($font['font-weight']) && !little_birdies_is_inherit($font['font-weight'])
														? 'font-weight:' . trim($font['font-weight']) . ";"
														: '';
				$rez[$tag.'_font-style'] 		= !empty($font['font-style']) && !little_birdies_is_inherit($font['font-style'])
														? 'font-style:' . trim($font['font-style']) . ";"
														: '';
				$rez[$tag.'_text-decoration'] 	= !empty($font['text-decoration']) && !little_birdies_is_inherit($font['text-decoration'])
														? 'text-decoration:' . trim($font['text-decoration']) . ";"
														: '';
				$rez[$tag.'_text-transform'] 	= !empty($font['text-transform']) && !little_birdies_is_inherit($font['text-transform'])
														? 'text-transform:' . trim($font['text-transform']) . ";"
														: '';
				$rez[$tag.'_letter-spacing'] 	= !empty($font['letter-spacing']) && !little_birdies_is_inherit($font['letter-spacing'])
														? 'letter-spacing:' . trim($font['letter-spacing']) . ";"
														: '';
				$rez[$tag.'_margin-top'] 		= !empty($font['margin-top']) && !little_birdies_is_inherit($font['margin-top'])
														? 'margin-top:' . little_birdies_prepare_css_value($font['margin-top']) . ";"
														: '';
				$rez[$tag.'_margin-bottom'] 	= !empty($font['margin-bottom']) && !little_birdies_is_inherit($font['margin-bottom'])
														? 'margin-bottom:' . little_birdies_prepare_css_value($font['margin-bottom']) . ";"
														: '';
			} else {
...
if (!function_exists('little_birdies_customizer_get_css')) {

	function little_birdies_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = little_birdies_customizer_add_theme_fonts($fonts);
...
CSS;
			$rez = apply_filters('little_birdies_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
		}

		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(little_birdies_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = little_birdies_get_scheme_colors($scheme);
...
.widget_product_search input.search_field {
...
a:hover .sc_layouts_item_details_line1 {
    color: {$colors['text']};
}
...
.scheme_self.sc_layouts_row_type_normal .sc_layouts_menu_nav > li li > a {
    color: {$colors['alter_dark']};
}
...
.sc_events_classic a:hover .sc_event_item_title {
    color: {$colors['alter_link']};
}
...
.little_birdies_tabs .little_birdies_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
.little_birdies_tabs .little_birdies_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
...
.scheme_self.footer_wrap h1, .scheme_self.footer_wrap h2, .scheme_self.footer_wrap h4, .scheme_self.footer_wrap h5, .scheme_self.footer_wrap h6,
...
CSS;
					...
					$rez = apply_filters('little_birdies_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'little_birdies_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>