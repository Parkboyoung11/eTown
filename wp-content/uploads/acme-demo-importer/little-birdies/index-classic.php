...
if (have_posts()) {
	...
	if (!$little_birdies_sticky_out) {
		if (little_birdies_get_theme_option('first_post_large') && !is_paged() && !in_array(little_birdies_get_theme_option('body_style'), array('fullwide', 'fullscreen'))) {
			the_post();
	...
	while ( have_posts() ) { the_post(); 
		...
		get_template_part( 'content', $little_birdies_sticky_out && is_sticky() ? 'sticky' :'classic' );
	}
